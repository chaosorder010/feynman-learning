...
- `feynman_validate_transition`: validates state transitions and Pi branch ownership before progress writes
- `feynman_record_score`: records scores and enforces the pass threshold
- `feynman_update_coach_memory`: appends evidence-backed long-term coach memory to `_learner/SOUL.md`
- `feynman_read_coach_memory`: reads long-term coach memory for cross-project personalization
- `feynman_retract_coach_memory`: retracts disproven or superseded coach memory while preserving audit history
...
- Mutating Feynman tools reject invalid state transitions and writes from non-descendant Pi session branches. Use `branchMode: "adopt"` only when the learner explicitly chooses the current branch as the canonical project path.
- `feynman_update_coach_memory` rejects unsupported memories: each entry must include concrete evidence and be learner-confirmed or observed at least twice.
- `feynman_read_coach_memory` omits retracted memories by default; retractions remain in `SOUL.md` only for auditability.
